...
async function checkedFetch(operation: string, input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
  const startedAt = performance.now();
  logEvent("plex.request.started", { operation, method: init?.method ?? "GET" }, "debug");
  try {
    const response = await fetch(input, init);
    const durationMs = Math.round(performance.now() - startedAt);
    logEvent("plex.request.completed", { operation, status: response.status, durationMs }, response.ok ? "debug" : "warn");
    if (!response.ok) throw new Error(`Plex request failed (${response.status})`);
    return response;
  } catch (reason) {
    logError("plex.request.failed", reason, { operation, durationMs: Math.round(performance.now() - startedAt) });
    throw reason;
  }
}
...
export async function createPlexPin(): Promise<PlexPin> {
  ...
  const response = await checkedFetch("pin.create", PIN_ENDPOINT, { method: "POST", headers: requestHeaders, body: new URLSearchParams({ strong: "true" }) });
...
export async function waitForPlexToken(pin: PlexPin, signal: AbortSignal): Promise<string> {
  ...
  while (!signal.aborted) {
    ...
    const response = await checkedFetch("pin.poll", url, { headers: headers(), signal });
...
export async function fetchPlexServers(token: string): Promise<PlexServer[]> {
  ...
  const response = await checkedFetch("server.discovery", url, { headers: headers(token) });
...
export async function fetchPlexLibraries(server: PlexServer): Promise<PlexLibrary[]> {
  ...
  const response = await checkedFetch("libraries.fetch", `${server.uri}/library/sections`, { headers: headers(server.accessToken) });
  const parsed = librarySchema.parse(await response.json());
  const libraries = parsed.MediaContainer.Directory.map((library) => ({ id: library.key, title: library.title, type: library.type }));
...
export async function fetchPlexMedia(server: PlexServer, libraries: readonly PlexLibrary[]): Promise<MediaItem[]> {
  ...
  const results = await Promise.all(libraries.map(async (library) => {
    const response = await checkedFetch("library.media.fetch", `${server.uri}/library/sections/${library.id}/all`, { headers: headers(server.accessToken) });
...
export async function applyPlexRating(server: PlexServer, mediaId: string, value: number | null): Promise<void> {
  ...
  await checkedFetch("rating.apply", url, { method: "PUT", headers: headers(server.accessToken) });
}
