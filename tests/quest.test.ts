...
describe("filterMedia", () => {
  it("selects watched and unrated quest modes", () => {
    expect(filterMedia(DEMO_MEDIA, "watched", DEFAULT_FILTERS)).toHaveLength(
      DEMO_MEDIA.length,
    );
    expect(
      filterMedia(DEMO_MEDIA, "unrated", DEFAULT_FILTERS).every(
  ...
  it("filters by media kind and watch count", () => {
    const result = filterMedia(DEMO_MEDIA, "movies", {
      ...DEFAULT_FILTERS,
      minimumWatchCount: 2,
    });
    expect(
      result.every((item) => item.kind === "movie" && item.watchCount >= 2),
    ).toBe(true);
  });

  it("never includes unwatched media in watched mode when the minimum is zero", () => {
    const source = DEMO_MEDIA[0];
    expect(source).toBeDefined();
    if (source === undefined) throw new Error("Demo media fixture is empty");
    const unwatched = { ...source, id: "never-watched", watchCount: 0 };
...
describe("quest summaries", () => {
  ...
  it("calculates rating and rerating statistics", () => {
    ...
    expect(calculateStats(DEMO_MEDIA, ratings, 3)).toEqual({
      count: 2,
      average: 9,
      fiveStarCount: 1,
      skips: 3,
  ...
  it("handles an empty completion", () => {
    expect(calculateStats([], {}, 0)).toEqual({
      count: 0,
      average: 0,
      fiveStarCount: 0,
      skips: 0,
